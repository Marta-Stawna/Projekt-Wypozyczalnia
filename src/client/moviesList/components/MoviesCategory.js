...
class MoviesCategory extends Component{
  ...
  render(){
    ...
    let category=this.props.category.filter((item,index)=>{
          return item.id==id
    })
    let filter=this.props.movies.filter((item,index)=>{
 ...
 export default connect(state=>({category:state.CategoryReducer.list,
