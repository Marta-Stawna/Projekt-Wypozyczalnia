...
class Basket extends Component{
  ...
  render(){
    ...
    <div className="panel panel-default">
      ...
      <div className="summary"><div> Zamowianio: {this.props.basket.length} szt </div><div> {this.props.basket.map((item)=>{
        {suma=suma+item.price}
      })} Cena : {suma} zł</div></div>
      </div>
      </div>
  <div ><Link to='/form' className="btn btn-primary btn-lg btn-block">Zamow</Link></div><hr/>
  </div>
    );
  }
}
